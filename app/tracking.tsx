...
import {
  ...
  Modal, Animated, Easing, StatusBar, Platform,
...
function getWeekActivity(logs) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const hasLog = logs.some((l) => new Date(l.date).toISOString().slice(0, 10) === iso);
    days.push({ label: ["S", "M", "T", "W", "T", "F", "S"][d.getDay()], active: hasLog, today: i === 0 });
  }
  return days;
}

function totalVolLog(log) {
  return log.exercises.reduce((sum, ex) => sum + ex.sets.reduce((s, set) => s + set.reps * set.weight, 0), 0);
}
...
function buildYearGrid(logs) {
  ...
  while (cursor <= today) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      else { const iso = cursor.toISOString().slice(0, 10); week.push({ date: iso, vol: volByDate[iso] || 0, hasLog: !!volByDate[iso] }); }
...
function MuscleAccordionRow({ mg, lastBest, delta, exNames, sessionCount, logs }) {
  ...
  logs.forEach((log) => {
    log.exercises.forEach((ex) => {
      if ((ex.muscleGroup || "").trim() === mg && !seen.has(ex.name)) { seen.add(ex.name); allExercises.push(ex); }
    });
  });
  return (
    <View style={ms.card}>
        ...
        <View style={ep.panel}>
          ...
          <ScrollView style={ep.list} showsVerticalScrollIndicator={false}>
            {list.map((name) => {
              ...
              return (
                <Pressable key={name} disabled={isAdded} onPress={() => !isAdded && toggle(name)}
                  style={[ep.item, isSel && ep.itemSel, isAdded && ep.itemAdded]}>
                  <Text style={[ep.itemText, isSel && ep.itemTextSel]}>{name}</Text>
                  {isAdded ? <Text style={ep.addedLabel}>Added</Text>
                    : isSel ? <Text style={{ color: "#ff6b35", fontWeight: "800", fontSize: 16 }}>✓</Text>
          ...
          <View style={ep.footer}>
            <Pressable disabled={picked.length === 0} onPress={() => onConfirm(picked, muscleGroup)}
              ...
              <Text style={ep.addBtnText}>Add {picked.length > 0 ? `${picked.length} exercise${picked.length > 1 ? "s" : ""}` : "exercises"}</Text>
            </Pressable>
          </View>
        </View>
      </View>
...
function LogSheet({ onClose, onSaved }) {
  ...
  const addSet = (ei) => setExercises((prev) => prev.map((ex, i) => {
    if (i !== ei) return ex;
  ...
  const onPickerConfirm = (names, mg) => {
    setExercises((prev) => [...prev, ...names.map((name) => ({ name, muscleGroup: mg, sets: [{ setNumber: 1, reps: 0, weight: 0 }] }))]);
    setPickerMg(null);
  };
  const submit = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:3000/api/tracking", {
        ...
        body: JSON.stringify({ date: new Date().toISOString(), exercises, notes }),
      });
          ...
          <View style={ls.panel}>
            ...
            {saved ? (
              <View style={ls.savedContainer}>
                <View style={ls.savedIcon}><Text style={{ fontSize: 24, color: "#22c55e" }}>✓</Text></View>
              ...
              <>
                <View style={ls.header}>
                  ...
                  <Pressable onPress={handleClose}><Text style={ls.closeBtn}>✕</Text></Pressable>
                </View>
                <ScrollView style={ls.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                  ...
                  <View style={ls.mgGrid}>
                    {MUSCLE_GROUPS.map((mg) => {
                      ...
                      return (
                        <Pressable key={mg} onPress={() => setPickerMg(mg)} style={[ls.mgBtn, count > 0 && ls.mgBtnActive]}>
                          <Text style={[ls.mgBtnText, count > 0 && ls.mgBtnTextActive]}>{mg}</Text>
                          {count > 0 && <View style={ls.mgCount}><Text style={ls.mgCountText}>{count}</Text></View>}
                        </Pressable>
                      );
                    })}
                  </View>
                  {exercises.length === 0 && (
                    <View style={ls.emptyCard}><Text style={ls.emptyText}>No exercises yet — tap a muscle group above</Text></View>
                  )}
                  {exercises.map((ex, ei) => (
                    <View key={ei} style={ls.exCard}>
                      ...
                      {ex.sets.map((set, si) => (
                        <View key={si} style={ls.setRow}>
                          ...
                          <TextInput style={ls.numInput} value={set.reps ? String(set.reps) : ""} placeholder="0" placeholderTextColor="#ccc" keyboardType="numeric" onChangeText={(v) => updateSet(ei, si, "reps", v)} />
                          <TextInput style={ls.numInput} value={set.weight ? String(set.weight) : ""} placeholder="0" placeholderTextColor="#ccc" keyboardType="decimal-pad" onChangeText={(v) => updateSet(ei, si, "weight", v)} />
                          <Pressable onPress={() => removeSet(ei, si)} style={ls.removeSetBtn}>
                            <Text style={ls.removeSetText}>−</Text>
                          </Pressable>
                        </View>
                      ))}
                      <Pressable onPress={() => addSet(ei)}><Text style={ls.addSetText}>+ Add set</Text></Pressable>
                    </View>
                  ))}
                  <TextInput style={ls.notesInput} placeholder="Notes (optional)…" placeholderTextColor="#aaa" multiline numberOfLines={2} value={notes} onChangeText={setNotes} />
                  <Pressable onPress={submit} disabled={saving} style={[ls.saveBtn, saving && ls.saveBtnDisabled]}>
                    <Text style={ls.saveBtnText}>{saving ? "Saving…" : `Save workout${exercises.length > 0 ? ` · ${exercises.length} exercise${exercises.length > 1 ? "s" : ""}` : ""}`}</Text>
                  </Pressable>
          ...
          <View key={log._id} style={hs.card}>
            <View style={hs.cardHeader}>
              ...
              <Pressable onPress={() => onDelete(log._id)} style={[hs.deleteBtn, isPending && hs.deleteBtnPending]}>
                <Text style={[hs.deleteBtnText, isPending && hs.deleteBtnTextPending]}>{isPending ? "Confirm?" : "Delete"}</Text>
              </Pressable>
            </View>
            ...
            {log.notes && <Text style={hs.notes}>"{log.notes}"</Text>}
          </View>
        );
      })}
      <View style={{ height: 140 }} />
...
export default function TrackingPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const token = await getToken();
      const res = await fetch("http://localhost:3000/api/tracking?limit=400", { headers: { Authorization: `Bearer ${token}` } });
      const json = await res.json();
  ...
  const deleteLog = async (id) => {
    ...
    const token = await getToken();
    await fetch(`http://localhost:3000/api/tracking?id=${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
  ...
  const weekVolume = logs.filter((l) => new Date(l.date) >= weekStart).reduce((s, l) => s + totalVolLog(l), 0);
  ...
  return (
    <View style={t.root}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {tab === "history" && <HistoryScreen logs={logs} loading={loadingLogs} onDelete={deleteLog} confirmDeleteId={confirmDeleteId} />}
      </View>
...
const t = StyleSheet.create({
  ...
  fab: { position: "absolute", bottom: 0, left: 0, right: 0, padding: 16, paddingBottom: Platform.OS === "ios" ? 32 : 16, backgroundColor: "transparent" },
