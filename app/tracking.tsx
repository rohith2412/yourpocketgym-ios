...
const toISODate = (date) => {
  return new Date(date).toLocaleDateString("en-CA"); // YYYY-MM-DD (LOCAL TIME)
};

function buildYearGrid(logs) {
  const volByDate = {};

  // 🔹 Aggregate volume per day (LOCAL date, not UTC)
  logs.forEach((log) => {
  ...
  // 🔹 Align to Sunday (GitHub style grid)
...
function LogSheet({ onClose, onSaved }) {
        ...
        return {
          ...
          sets: [
            ...ex.sets,
            {
              ...
              // ✅ carry over last set values as strings for the input
              reps: last ? last.reps : "",
  ...
  // ✅ FIX: keep value as string while typing, don't force Number()
  ...
  const onPickerConfirm = (names, mg) => {
    setExercises((prev) => [
      ...
      ...names.map((name) => ({
        name,
        muscleGroup: mg,
        // ✅ start with empty strings so placeholder shows
  ...
  const submit = async () => {
    ...
    // ✅ FIX: warn if any set is still 0 — this is why calendar was blank
    const hasZero = cleanedExercises.some((ex) =>
      ex.sets.some((s) => s.reps === 0 || s.weight === 0),
    );
    if (hasZero) {
      alert("Some sets are missing reps or weight. Please fill them all in.");
      return;
    }
    ...
    try {
      ...
      // ✅ FIX: correct URL (was "yourpocketgym.comapi" before)
  ...
  return (
    ...
    >
      ...
      <Animated.View
        style={[ls.panelWrapper, { transform: [{ translateY: slideAnim }] }]}
      >
        <KeyboardAvoidingView
                    ...
                    <View key={ei} style={ls.exCard}>
                      ...
                      <View style={ls.setHeader}>
                        {["SET", "REPS", "LBS", ""].map((lbl, i) => (
                          <Text
                            key={i}
                            style={[ls.setHeaderText, i === 3 && { width: 32 }]}
                          >
                            {lbl}
                          </Text>
                        ))}
                      ...
                      {ex.sets.map((set, si) => (
                        <View key={si} style={ls.setRow}>
                          ...
                          <TextInput
                            style={ls.numInput}
                            // ✅ FIX: value stays as string, no forced Number()
                            value={String(set.reps)}
                            placeholder="0"
                            placeholderTextColor="#ccc"
                            keyboardType="numeric"
                            onChangeText={(v) =>
                              updateSet(ei, si, "reps", v.replace(/[^0-9]/g, ""))
                            }
                          />
                          <TextInput
                            style={ls.numInput}
...
function HistoryScreen({ logs, loading, onDelete, confirmDeleteId }) {
  if (loading)
    return <ActivityIndicator style={{ padding: 60 }} color="#ff6b35" />;
  if (!logs.length) {
    return (
      <View style={hs.empty}>
