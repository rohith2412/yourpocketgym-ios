...
const av = StyleSheet.create({
  circle: { backgroundColor: "#1a1a1a", alignItems: "center", justifyContent: "center" },
    ...
    <View style={es.group}>
      ...
      <View style={es.chips}>
        {options.map((o) => (
          ...
          >
            <Text style={[es.chipText, form[field] === o.val && es.chipTextActive]}>
              {o.icon} {o.label}
            </Text>
          </Pressable>
        ))}
    ...
    <Modal visible animationType="slide" transparent onRequestClose={onClose}>
      ...
      <View style={es.sheet}>
        ...
        <ScrollView style={es.body} showsVerticalScrollIndicator={false}>
          <Chips
            label="Biological sex" field="gender"
            options={[
          ...
          <Chips
            label="Main goal" field="fitnessGoal"
            options={[
          ...
          />
          <Chips
            label="Experience" field="experienceLevel"
            options={[
          ...
          <View style={es.group}>
            ...
            <View style={es.daysRow}>
              {[1,2,3,4,5,6,7].map((n) => (
                <Pressable
                  key={n}
                  style={[es.dayBtn, form.workoutDaysPerWeek === n && es.dayBtnActive]}
                  onPress={() => set("workoutDaysPerWeek", n)}
                >
                  <Text style={[es.dayBtnText, form.workoutDaysPerWeek === n && es.dayBtnTextActive]}>
                    {n}
                  </Text>
                </Pressable>
              ))}
...
const es = StyleSheet.create({
  ...
  handle: {
    width: 36, height: 4, backgroundColor: "#e8e5de",
    borderRadius: 99, alignSelf: "center", marginTop: 12,
  },
  head: {
    flexDirection: "row", alignItems: "center",
    justifyContent: "space-between", padding: 16,
    borderBottomWidth: 1, borderBottomColor: "#e8e5de",
  },
...
export default function ProfileScreen() {
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) {
        setProfile((p) => ({ ...p, subscriptionStatus: "canceling", isSubscribed: false }));
  ...
  const daysRemaining = profile?.currentPeriodEnd
    ? Math.max(0, Math.ceil((new Date(profile.currentPeriodEnd) - Date.now()) / 86400000))
    : null;
      ...
      >
        ...
        <Card style={s.heroCard}>
          ...
          <View style={s.heroInfo}>
            ...
            {profile?.memberSince && (
              <Text style={s.memberSince}>Member since {formatDate(profile.memberSince)}</Text>
            )}
            <View style={s.pillsRow}>
              ...
              {profile?.age > 0 && (
                <View style={s.pill}><Text style={s.pillText}>{profile.age} yrs</Text></View>
              )}
              {profile?.workoutDaysPerWeek > 0 && (
                <View style={s.pill}><Text style={s.pillText}>{profile.workoutDaysPerWeek}×/wk</Text></View>
              )}
            </View>
          </View>
        </Card>
          ...
          <View style={s.proBanner}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.proBannerSub}>
                {expiryDate ? `Renews · ${expiryDate}` : "All features unlocked"}
              </Text>
            </View>
        ...
        {profile && !profile.isSubscribed && profile.subscriptionStatus !== "canceling" && (
          <Pressable style={s.upgradeBanner} onPress={() => router.push("/pricing")}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.upgradeSub}>AI trainer, nutrition tracking, recipes & more</Text>
            </View>
              ...
              <Card style={{ marginBottom: 10 }}>
                ...
                <View style={s.bmiScale}>
                  {["10", "18.5", "25", "30", "40"].map((n) => (
                    <Text key={n} style={s.bmiScaleText}>{n}</Text>
                  ))}
                </View>
              </Card>
            )}
            ...
            <>
              ...
              {cancelConfirm && expiryDate && (
                <View style={s.cancelWarning}>
                  <Text style={s.cancelWarningTitle}>⚠️ You'll lose Pro access on:</Text>
...
const s = StyleSheet.create({
  ...
  card: {
    ...
    borderWidth: 1, borderColor: "#e8e5de", marginBottom: 10,
  },
  ...
  pill:         { paddingVertical: 3, paddingHorizontal: 9, borderRadius: 99, backgroundColor: "#f4f2ed", borderWidth: 1, borderColor: "#e8e5de" },
  pillText:     { fontSize: 11, color: "#666", fontWeight: "500", textTransform: "capitalize" },
  ...
  proBanner:    { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "rgba(255,107,53,0.07)", borderWidth: 1, borderColor: "rgba(255,107,53,0.2)", borderRadius: 16, padding: 14, marginBottom: 10 },
  ...
  upgradeBanner:  { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#1a1a1a", borderRadius: 16, padding: 14, marginBottom: 10 },
  ...
  cancelNotice:   { flexDirection: "row", gap: 8, alignItems: "flex-start", backgroundColor: "rgba(245,158,11,0.08)", borderWidth: 1, borderColor: "rgba(245,158,11,0.25)", borderRadius: 14, padding: 14, marginBottom: 10 },
  cancelNoticeText: { fontSize: 12, color: "#d97706", fontWeight: "600", lineHeight: 18, flex: 1 },
  ...
  trainingCard: { flex: 1, flexDirection: "row", alignItems: "center", gap: 10, padding: 14 },
  trainingLabel:{ fontSize: 13, fontWeight: "700", color: "#1a1a1a", marginTop: 2 },
  noIntroCard:  { alignItems: "center", padding: 32, marginBottom: 10 },
  noIntroTitle: { fontSize: 14, fontWeight: "600", color: "#1a1a1a", marginBottom: 6 },
  ...
  deleteLink: { fontSize: 11, color: "#bbb", textDecorationLine: "underline", textAlign: "center", marginBottom: 8 },
  version:    { textAlign: "center", fontSize: 10, color: "#ccc", fontWeight: "500", letterSpacing: 1 },
});