...
import {
  ...
  StyleSheet, Animated, ActivityIndicator, KeyboardAvoidingView,
    ...
    <View style={s.dayCard}>
      <Pressable onPress={() => setOpen(o => !o)} style={s.dayCardHeader}>
        <View style={[s.dayBox, open && s.dayBoxOpen]}>
          ...
          <Text style={[s.dayBoxNum, open && { color: "#fff" }]}>{index + 1}</Text>
        </View>
        <View style={{ flex: 1 }}>
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 12, paddingBottom: 40 }}>
      ...
      <View style={s.actionRow}>
        ...
        {onRegen && (
          <Pressable onPress={onRegen} disabled={isLoading}
      ...
      <View style={s.segControl}>
        {[["schedule", "Schedule"], ["tips", "Tips & Nutrition"]].map(([key, label]) => (
          <Pressable key={key} onPress={() => setTab(key)}
            ...
            <Text style={[s.segBtnText, tab === key && s.segBtnTextActive]}>{label}</Text>
          </Pressable>
        ))}
      </View>
          ...
          {plan.progressionTips?.length > 0 && (
            <View style={s.card}>
              ...
              {plan.progressionTips.map((t, i) => (
                <View key={i} style={s.tipRow}>
                  <View style={[s.tipNum, { backgroundColor: "rgba(99,102,241,0.1)" }]}>
                    <Text style={[s.tipNumText, { color: "#6366f1" }]}>{i + 1}</Text>
                  </View>
          ...
          {plan.nutritionTips?.length > 0 && (
            <View style={s.card}>
              ...
              {plan.nutritionTips.map((t, i) => (
                <View key={i} style={s.tipRow}>
                  <View style={[s.tipNum, { backgroundColor: "rgba(34,197,94,0.1)" }]}>
                    <Text style={[s.tipNumText, { color: "#22c55e" }]}>{i + 1}</Text>
                  </View>
                  <Text style={s.tipText}>{t}</Text>
                </View>
              ))}
  ...
  return (
    <View style={s.card}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        <Text style={{ fontSize: 12, color: "#bbb" }}>{fmtDate(saved.savedAt)}</Text>
      </View>
      <Text style={s.savedPlanTitle}>{saved.plan.planTitle}</Text>
      <Text style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>{saved.plan.weeklyVolume}</Text>
      <View style={{ flexDirection: "row", gap: 8, marginTop: 14 }}>
        <Pressable onPress={() => onView(saved)} style={s.viewBtn}>
          <Text style={s.viewBtnText}>View plan</Text>
        </Pressable>
        <Pressable onPress={() => onRemove(saved._id)} disabled={removing}
...
function ExtraForm({ extra, onChange, onClose }) {
  ...
  const toggleMulti = (field, val) => {
    const arr  = (local[field] || "").split(",").map(s => s.trim()).filter(Boolean);
    const next = arr.includes(val) ? arr.filter(v => v !== val) : [...arr, val];
    setLocal(p => ({ ...p, [field]: next.join(", ") }));
  };

  const isMulti = (field, val) =>
    (local[field] || "").split(",").map(s => s.trim()).includes(val);
    ...
    <Modal transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={s.sheet}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20 }}>
          ...
          <View style={s.chipRow}>
            {FOCUS_OPTIONS.map(f => (
              <Chip key={f} label={f} active={isMulti("focusAreas", f)} onPress={() => toggleMulti("focusAreas", f)} />
            ))}
          </View>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            {SESSION_LENGTHS.map(l => (
              <Pressable key={l} onPress={() => toggle("sessionLength", l)}
          ...
          <Text style={[s.sheetLabel, { marginTop: 20 }]}>Injuries / limitations</Text>
          <TextInput
          ...
          <Pressable onPress={() => { onChange(local); onClose(); }} style={[s.saveSheetBtn, { marginTop: 28 }]}>
...
export default function AITrainerScreen() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/api/user-intro`, {
      ...
      .then(r => r.json())
      .then(json => { if (json.success && json.data) setIntro(json.data); })
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/api/ai-trainer/saved`, {
      ...
      .then(r => r.json())
      .then(json => { if (json.success) setSavedPlans(json.data); })
      .catch(() => {})
  ...
  async function generatePlan(forceNew = false) {
    ...
    try {
      ...
      setPlan(json.data); setShowPlan(true);
    } catch (e) {
    ...
    <SafeAreaView style={s.screen} edges={["top"]}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 20 }}>
        ...
        {showTabs && mainTab !== "chat" && introReady && (
          <ProfileBanner intro={intro} extra={extra} onEdit={() => setShowExtra(true)} />
        )}

        {/* Plan tab — landing edit */}
        {mainTab === "plan" && !showPlan && (
          <ScrollView showsVerticalScrollIndicator={false}
            contentContainerStyle={{ paddingTop: 4, paddingBottom: 40, gap: 14 }}>

            <View style={s.planLandingCard}>
              <Text style={s.planLandingTitle}>Your weekly plan</Text>
              <Text style={s.planLandingDesc}>
                AI builds a full programme based on your profile, goals and equipment.
              </Text>
              {planError && <Text style={s.errorText}>{planError}</Text>}
              <Pressable onPress={() => generatePlan(false)} disabled={planLoad}
            ...
            {planLoad ? (
              <View style={{ gap: 10 }}>
                {[120, 72, 72, 72, 72].map((h, i) => <Skeleton key={i} height={h} />)}
              </View>
            ) : (
              <View style={{ gap: 8 }}>
                {[
                  { title: "Full 7-day schedule",  desc: "Training days and rest days tailored to your availability" },
                  { title: "Exercise breakdown",   desc: "Sets, reps, rest periods and tempo for every movement" },
                  { title: "Progression system",   desc: "How to add load and intensity each week" },
                  { title: "Nutrition guidelines", desc: "What to eat to support your specific goal" },
                  { title: "Warmup & cooldown",    desc: "Built into every training day" },
                ].map((f, i) => (
                  <View key={i} style={[s.card, { flexDirection: "row", gap: 14, padding: 16, alignItems: "flex-start" }]}>
                    <View style={s.featureNum}><Text style={s.featureNumText}>{i + 1}</Text></View>
                    <View style={{ flex: 1 }}>
        ...
        {mainTab === "plan" && showPlan && plan && (
          <PlanView plan={plan} onRegen={() => generatePlan(true)} isLoading={planLoad}
            onBack={() => setShowPlan(false)} onSave={savePlan}
            isSaved={!!savedId} saving={saving} />
        )}

        {/* Saved tab */}
        {mainTab === "saved" && !viewingSaved && (
          <ScrollView showsVerticalScrollIndicator={false}
            contentContainerStyle={{ paddingTop: 4, paddingBottom: 40, gap: 10 }}>
            {savedLoading ? (
              <View style={{ gap: 10 }}>
                {[90, 90, 90].map((h, i) => <Skeleton key={i} height={h} />)}
              </View>
            ) : savedPlans.length === 0 ? (
              <View style={[s.card, { alignItems: "center", padding: 48 }]}>
                <Text style={s.emptyTitle}>No saved plans</Text>
                <Text style={s.emptyDesc}>Generate a plan and tap Save to keep it here.</Text>
              </View>
            ) : (
              savedPlans.map(sv => (
                <SavedPlanCard key={sv._id} saved={sv}
                  onView={item => setViewingSaved(item)}
                  onRemove={removePlan} removing={removingId === sv._id} />
              ))
            )}
          </ScrollView>
        )}
        ...
        {/* Saved plan viewer */}
        {mainTab === "saved" && viewingSaved && (
          <PlanView plan={viewingSaved.plan} onRegen={null} isLoading={false}
            onBack={() => setViewingSaved(null)} onSave={null}
            isSaved={true} saving={false} />
        )}
      ...
      {showExtra && (
        <ExtraForm extra={extra} onChange={setExtra} onClose={() => setShowExtra(false)} />
      )}
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  // Header
  header:       { paddingHorizontal: 20, paddingTop: 10, paddingBottom: 0, borderBottomWidth: 1, borderBottomColor: "rgba(232,229,222,0.6)", backgroundColor: "#fafaf8" },
  ...
  avatarBtn:     { width: 40, height: 40, borderRadius: 20, backgroundColor: "#1a1a1a", alignItems: "center", justifyContent: "center" },
  ...
  tabBadge:      { position: "absolute", top: 8, right: 8, minWidth: 16, height: 16, borderRadius: 8, backgroundColor: "#ff6b35", alignItems: "center", justifyContent: "center", paddingHorizontal: 4 },
  tabBadgeText:  { fontSize: 9, fontWeight: "800", color: "#fff" },

  // Profile banner — fixed alignment
  profileBanner:       { flexDirection: "row", alignItems: "center", backgroundColor: "#1a1a1a", borderRadius: 18, paddingVertical: 14, paddingHorizontal: 16, marginTop: 14, marginBottom: 4 },
  profileBannerLeft:   { flex: 1, marginRight: 12 },
  profileBannerEyebrow:{ fontSize: 10, fontWeight: "700", color: "rgba(255,255,255,0.35)", letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 },
  profileTags:         { flexDirection: "row", flexWrap: "wrap", gap: 6 },
  profileTag:          { backgroundColor: "rgba(255,255,255,0.1)", borderRadius: 99, paddingHorizontal: 10, paddingVertical: 4 },
  profileTagText:      { fontSize: 12, fontWeight: "600", color: "rgba(255,255,255,0.85)" },
  profileEditBtn: {
    flexDirection: "row",
    alignItems: "center",
    ...
    paddingVertical: 7,
  },  profileEditBtnText:  { fontSize: 12, fontWeight: "700", color: "rgba(255,255,255,0.6)" },

  // Cards
  card:          { backgroundColor: "#fff", borderRadius: 18, borderWidth: 1, borderColor: "#e8e5de", padding: 18, shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } },

  // Plan landing
  planLandingCard:  { backgroundColor: "#fff", borderRadius: 20, borderWidth: 1, borderColor: "#e8e5de", padding: 24, alignItems: "center", marginTop: 4 },
  planLandingTitle: { fontSize: 20, fontWeight: "800", color: "#1a1a1a", letterSpacing: -0.5, marginBottom: 8, textAlign: "center" },
  planLandingDesc:  { fontSize: 14, color: "#aaa", lineHeight: 22, marginBottom: 22, textAlign: "center" },
  errorText:        { fontSize: 13, color: "#ef4444", fontWeight: "600", marginBottom: 12 },
  generateBtn:      { width: "100%", backgroundColor: "#1a1a1a", borderRadius: 14, paddingVertical: 16, alignItems: "center" },
  generateBtnText:  { fontSize: 15, fontWeight: "700", color: "#fff" },

  // Feature list
  featureNum:     { width: 28, height: 28, borderRadius: 8, backgroundColor: "#f4f2ed", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  chip:          { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 99, borderWidth: 1.5, borderColor: "#e8e5de", backgroundColor: "#fff" },
  ...
  actionBtnPrimary:   { flex: 1, paddingVertical: 12, backgroundColor: "#ff6b35", borderRadius: 12, alignItems: "center", justifyContent: "center" },
  ...
  planSummary:   { fontSize: 13, color: "rgba(255,255,255,0.55)", lineHeight: 20, marginVertical: 10 },
  ...
  focusBadge:    { backgroundColor: "rgba(255,107,53,0.08)", borderRadius: 99, paddingHorizontal: 10, paddingVertical: 4 },
  ...
  warmupLabel:   { fontSize: 10, fontWeight: "700", textTransform: "uppercase", letterSpacing: 1, color: "#ff6b35", marginBottom: 4 },
  ...
  restDayCard:  { flexDirection: "row", alignItems: "center", gap: 14, padding: 16, backgroundColor: "#fff", borderWidth: 1, borderColor: "#e8e5de", borderRadius: 18 },
  restDayBox:   { width: 46, height: 46, borderRadius: 13, backgroundColor: "#f4f2ed", alignItems: "center", justifyContent: "center" },
  restDayLabel: { fontSize: 9, fontWeight: "800", color: "#bbb", letterSpacing: 1 },
  ...
  tempoBadge:    { backgroundColor: "rgba(255,107,53,0.08)", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  tipNum:          { width: 22, height: 22, borderRadius: 6, backgroundColor: "rgba(255,107,53,0.08)", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  tipNumText:      { fontSize: 11, fontWeight: "800", color: "#ff6b35" },
  tipText:         { fontSize: 14, color: "#555", lineHeight: 21, flex: 1 },

  // Saved plan card
  savedPlanTitle: { fontSize: 16, fontWeight: "800", color: "#1a1a1a", letterSpacing: -0.3, marginTop: 4 },
  viewBtn:        { flex: 1, paddingVertical: 11, backgroundColor: "#1a1a1a", borderRadius: 12, alignItems: "center" },
  viewBtnText:    { fontSize: 13, fontWeight: "700", color: "#fff" },
  removeBtn:      { paddingHorizontal: 16, paddingVertical: 11, backgroundColor: "rgba(239,68,68,0.06)", borderWidth: 1, borderColor: "rgba(239,68,68,0.15)", borderRadius: 12, alignItems: "center" },
  removeBtnText:  { fontSize: 13, fontWeight: "700", color: "#ef4444" },

  // Empty
  emptyTitle: { fontSize: 16, fontWeight: "700", color: "#1a1a1a", marginBottom: 6 },
  emptyDesc:  { fontSize: 14, color: "#aaa", lineHeight: 21, textAlign: "center" },
  ...
  sheet:          { position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#fafaf8", borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: "88%", shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 20, shadowOffset: { width: 0, height: -4 }, elevation: 20 },
  sheetHandle:    { width: 36, height: 4, borderRadius: 2, backgroundColor: "#e0ddd6", alignSelf: "center", marginTop: 14 },
  sheetTitle:     { fontSize: 20, fontWeight: "800", color: "#1a1a1a", letterSpacing: -0.5, marginBottom: 20 },
  sheetLabel:     { fontSize: 11, fontWeight: "700", letterSpacing: 1, textTransform: "uppercase", color: "#bbb", marginBottom: 10 },
  sessBtn:        { flex: 1, paddingVertical: 11, borderRadius: 11, borderWidth: 1.5, borderColor: "#e8e5de", backgroundColor: "#fff", alignItems: "center" },
  sessBtnActive:  { backgroundColor: "#1a1a1a", borderColor: "#1a1a1a" },
  sessBtnText:    { fontSize: 13, fontWeight: "700", color: "#aaa" },
  injuryInput:    { paddingHorizontal: 16, paddingVertical: 13, borderWidth: 1.5, borderColor: "#e8e5de", borderRadius: 13, fontSize: 14, color: "#1a1a1a", backgroundColor: "#fff" },
  saveSheetBtn:   { backgroundColor: "#1a1a1a", borderRadius: 14, paddingVertical: 16, alignItems: "center" },
